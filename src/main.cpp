...
unsigned int nStakeMaxAge = 30 * 24 * 60 * 60 * 12;// 360 days
...
const string strMessageMagic = "REGALCOIN Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+4) - GetDepthInMainChain());
}
...
static const int64_t nTargetTimespan = 24 * 60 *60 *30 *12;  // 360 
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "REGALCOIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "REGALCOIN Official Source";
        CTransaction txNew;
        txNew.nTime = 1506468888;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1506468888;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 838334;

        //// debug print
        assert(block.hashMerkleRoot == uint256("0x1a515d97191ac218f02b7526080e61d6f6bacb54ec2ba54ba4b9376f55c42c31"));
        block.print();
